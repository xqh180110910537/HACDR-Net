...
class DDHA(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(DDHA, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x = norm(x)
            x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            # from tools.feature_visualization2 import draw_feature_map
            # if count == 3: draw_feature_map(x)
            outs.append(x)
